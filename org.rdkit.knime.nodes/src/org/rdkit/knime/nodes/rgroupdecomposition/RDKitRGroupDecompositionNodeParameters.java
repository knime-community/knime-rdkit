...
final class RDKitRGroupDecompositionNodeParameters implements NodeParameters {
    ...
    @ValueReference(IsSDFCompatibleOrAdaptable.class)
    boolean m_strictParsingEnabled;
    
    static final class IsSDFCompatibleOrAdaptable implements BooleanReference {
	}
    ...
    @Effect(predicate = IsSDFCompatibleOrAdaptable.class, type = EffectType.SHOW)
    @Effect(predicate = HasSecondInputTable.class, type = EffectType.SHOW)
    ...
    @Widget(title = "Labels to recognize R-Groups in scaffolds", description = """
    		Enable one or multiple labels to be used to recognize R-Groups in scaffolds. At least one item 
    		needs to be picked.
    		""", advanced = true)
    ...
    static final class HasSecondInputTableAndIsSDFOnlyCoreInputColumn implements EffectPredicateProvider {
        ...
        @Override
        public EffectPredicate init(final PredicateInitializer i) {
            return i.getPredicate(HasSecondInputTable.class)
            		.and(i.getPredicate(IsSDFCompatibleOrAdaptable.class));
        }
    ...
    static final class LabelsChoicesProvider implements StringChoicesProvider {
        ...
        public List<StringChoice> computeState(final NodeParametersInput context) {
            return Arrays.stream(Labels.values())
                .map(l -> new StringChoice(l.name(), l.getLabel()))
                .toList();
        }
