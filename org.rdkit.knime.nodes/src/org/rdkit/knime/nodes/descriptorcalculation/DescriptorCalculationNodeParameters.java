...
final class DescriptorCalculationNodeParameters implements NodeParameters {
	...
	static final class DescriptorMigration implements NodeParametersMigration<String[]> {

		@Override
		public List<ConfigMigration<String[]>> getConfigMigrations() {
			return List.of(ConfigMigration.builder(settings -> getProperDescriptors(settings))
					.withMatcher(s -> s.containsKey("selectedDescriptors")).build());
		}

		private String[] getProperDescriptors(final NodeSettingsRO settings) {
    ...
    static final class DescriptorChoicesProvider implements StringChoicesProvider {
        ...
        public List<StringChoice> computeState(final NodeParametersInput context) {
            return Arrays.stream(Descriptor.class.getEnumConstants())
            	.filter(d -> (d != null && !d.name().startsWith("FlowVariablePlaceHolder")))
                .map(d -> new StringChoice(d.name(), d.toString()))
                .toList();
        }
